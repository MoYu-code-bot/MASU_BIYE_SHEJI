...
# 设置图片保存路径
IMAGE_SAVE_PATH = r"D:\Masu_BiYe_sheji\Python_SuanFa\electric_car_sale\利润过户分析算法图片存储"
...
def load_profit_transfer_data():
    ...
    # 加载数据
    try:
        profit_df = pd.read_excel(r"D:\HuaweiMoveData\Users\江磊\Desktop\电动汽车销售\AI文件\利润分析.xlsx")
        print(f"  ✓ 利润数据: {len(profit_df)} 条记录")
        print(f"    利润数据列名: {list(profit_df.columns)}")

        # 数据清洗
        profit_df = profit_df.dropna(subset=['total_profit', 'total_revenue'])  # 删除关键列空值
        profit_df = profit_df[profit_df['total_revenue'] > 0]  # 过滤收入为0的记录
    ...
    try:
        transfer_df = pd.read_excel(r"D:\HuaweiMoveData\Users\江磊\Desktop\电动汽车销售\AI文件\过户状态分析.xlsx")
        print(f"  ✓ 过户数据: {len(transfer_df)} 条记录")
        print(f"    过户数据列名: {list(transfer_df.columns)}")

        # 数据清洗
        transfer_df = transfer_df.dropna(subset=['transfer_count', 'avg_transfer_price'])  # 删除关键列空值
        transfer_df = transfer_df[transfer_df['transfer_count'] >= 0]  # 过滤过户数量为负的记录
...
def profit_composition_analysis(df):
    ...
    # 利润指标计算（确保数据完整性）
    if profit_margin_col not in df.columns:
        df['profit_margin'] = (df[profit_col] / df[revenue_col]) * 100  # 计算利润率
    else:
        df['profit_margin'] = df[profit_margin_col]

    if total_cost_col:
        df['cost_ratio'] = (df[total_cost_col] / df[revenue_col]) * 100  # 成本率
        df['cost_profit_ratio'] = df[total_cost_col] / df[profit_col]  # 成本利润比
    else:
...
def build_profit_prediction_model(df):
    """
    构建利润预测模型（适配实际数据结构）
    ...
    """
    if df.empty or not profit_insights:
...
def build_transfer_price_model(df):
    """
    构建过户价格预测模型（适配实际数据结构）
    ...
    返回:
        ...
        results: dict - 预测结果和评估指标
    """
    if df.empty or not transfer_insights:
...
def plot_profit_transfer_analysis():
    ...
    if profit_insights and not profit_insights.get('brand_profit', pd.DataFrame()).empty:
        ...
        # 显式指定字体
        fig.suptitle('利润分析图表', fontsize=16, fontweight='bold', fontfamily='Microsoft YaHei')
        ...
        axes[0, 0].set_yticklabels(top_10_brands.index, fontfamily='Microsoft YaHei')
        axes[0, 0].set_xlabel('总利润 (¥)', fontfamily='Microsoft YaHei')
        axes[0, 0].set_title('品牌利润排名（前10）', fontfamily='Microsoft YaHei')
        ...
        for i, bar in enumerate(bars):
            ...
            axes[0, 0].text(width + width * 0.01, bar.get_y() + bar.get_height() / 2,
                            f'¥{width / 1e6:.1f}M', ha='left', va='center', fontsize=9, fontfamily='Microsoft YaHei')
        ...
        axes[0, 1].set_xlabel('利润率 (%)', fontfamily='Microsoft YaHei')
        axes[0, 1].set_ylabel('频数', fontfamily='Microsoft YaHei')
        axes[0, 1].set_title('利润率分布', fontfamily='Microsoft YaHei')
        axes[0, 1].legend(prop={'family': 'Microsoft YaHei'})
        ...
        if 'cost_ratio' in profit_data.columns and not profit_data['cost_ratio'].isna().all():
            ...
            axes[1, 0].set_xlabel('成本率 (%)', fontfamily='Microsoft YaHei')
            axes[1, 0].set_ylabel('利润率 (%)', fontfamily='Microsoft YaHei')
            axes[1, 0].set_title('成本率 vs 利润率', fontfamily='Microsoft YaHei')
            ...
            axes[1, 0].set_xticklabels(top_10_brands.index, rotation=45, ha='right', fontfamily='Microsoft YaHei')
            axes[1, 0].set_ylabel('平均利润率 (%)', fontfamily='Microsoft YaHei')
            axes[1, 0].set_title('品牌平均利润率对比', fontfamily='Microsoft YaHei')
            axes[1, 0].grid(True, alpha=0.3)
        ...
        if not monthly_profit.empty:
            ...
            if '年份' in monthly_profit.columns and '月份' in monthly_profit.columns:
                ...
                axes[1, 1].plot(dates, monthly_profit['月度总利润'], marker='o', linewidth=2, markersize=4,
                                color='darkblue')
                axes[1, 1].set_xlabel('日期', fontfamily='Microsoft YaHei')
                axes[1, 1].set_ylabel('月度总利润 (¥)', fontfamily='Microsoft YaHei')
                axes[1, 1].set_title('月度利润趋势', fontfamily='Microsoft YaHei')
                axes[1, 1].tick_params(axis='x', rotation=45)
                axes[1, 1].grid(True, alpha=0.3)
            else:
                axes[1, 1].text(0.5, 0.5, '月度数据\n格式错误', ha='center', va='center',
                                transform=axes[1, 1].transAxes, fontsize=14, fontfamily='Microsoft YaHei')
                axes[1, 1].set_title('月度利润趋势', fontfamily='Microsoft YaHei')
        else:
    ...
    if transfer_insights and not transfer_insights.get('brand_transfer', pd.DataFrame()).empty:
        ...
        fig, axes = plt.subplots(2, 2, figsize=(16, 12))
        fig.suptitle('过户状态分析图表', fontsize=16, fontweight='bold', fontfamily='Microsoft YaHei')
        ...
        axes[0, 0].set_yticks(range(len(top_10_brands)))
        axes[0, 0].set_yticklabels(top_10_brands.index, fontfamily='Microsoft YaHei')
        axes[0, 0].set_xlabel('总过户数量 (辆)', fontfamily='Microsoft YaHei')
        axes[0, 0].set_title('品牌过户数量排名（前10）', fontfamily='Microsoft YaHei')
        axes[0, 0].invert_yaxis()
        ...
        for i, bar in enumerate(bars):
            width = bar.get_width()
            axes[0, 0].text(width + width * 0.01, bar.get_y() + bar.get_height() / 2,
                            f'{int(width):,}', ha='left', va='center', fontsize=9, fontfamily='Microsoft YaHei')
        ...
        if not transfer_status.empty:
            axes[0, 1].pie(transfer_status.values, labels=transfer_status.index,
                           autopct='%1.1f%%', startangle=90, textprops={'fontfamily': 'Microsoft YaHei'})
            axes[0, 1].set_title('过户状态分布', fontfamily='Microsoft YaHei')
        else:
            # 品牌平均过户价格
            axes[0, 1].bar(range(len(top_10_brands)), top_10_brands['平均过户价格'] / 1e4,
                           color=plt.cm.plasma(np.linspace(0, 1, len(top_10_brands))))
            axes[0, 1].set_xticks(range(len(top_10_brands)))
            axes[0, 1].set_xticklabels(top_10_brands.index, rotation=45, ha='right', fontfamily='Microsoft YaHei')
            axes[0, 1].set_ylabel('平均过户价格 (万元)', fontfamily='Microsoft YaHei')
            axes[0, 1].set_title('品牌平均过户价格对比', fontfamily='Microsoft YaHei')
        ...
        if not transfer_reasons.empty:
            ...
            axes[1, 0].set_xlabel('过户原因', fontfamily='Microsoft YaHei')
            axes[1, 0].set_ylabel('数量', fontfamily='Microsoft YaHei')
            axes[1, 0].set_title('过户原因分布', fontfamily='Microsoft YaHei')
            axes[1, 0].tick_params(axis='x', rotation=45)
            axes[1, 0].grid(True, alpha=0.3)
        else:
            axes[1, 0].text(0.5, 0.5, '过户原因数据\n不可用', ha='center', va='center',
                            transform=axes[1, 0].transAxes, fontsize=14, fontfamily='Microsoft YaHei')
            axes[1, 0].set_title('过户原因分布', fontfamily='Microsoft YaHei')
        ...
        if not monthly_transfers.empty:
            if '年份' in monthly_transfers.columns and '月份' in monthly_transfers.columns:
                ...
                axes[1, 1].set_xlabel('日期', fontfamily='Microsoft YaHei')
                axes[1, 1].set_ylabel('月度过户总量 (辆)', fontfamily='Microsoft YaHei')
                axes[1, 1].set_title('月度过户量趋势', fontfamily='Microsoft YaHei')
                axes[1, 1].tick_params(axis='x', rotation=45)
                axes[1, 1].grid(True, alpha=0.3)
            else:
                axes[1, 1].text(0.5, 0.5, '月度数据\n格式错误', ha='center', va='center',
...
# 保存报告到文件
try:
    with open(f"{IMAGE_SAVE_PATH}\\利润过户分析报告.txt", 'w', encoding='utf-8') as f:
        f.write(report_content)
    print(f"\n✓ 分析报告已保存: {IMAGE_SAVE_PATH}\\利润过户分析报告.txt")
except Exception as e:
    print(f"\n✗ 保存报告失败: {e}")
...
try:
    if profit_insights and not profit_insights.get('brand_profit', pd.DataFrame()).empty:
        ...
        brand_profit_df.to_csv(f"{IMAGE_SAVE_PATH}\\品牌利润分析.csv", encoding='utf-8-sig')
        print("  ✓ 品牌利润分析数据已保存")

    if transfer_insights and not transfer_insights.get('brand_transfer', pd.DataFrame()).empty:
        brand_transfer_df = transfer_insights['brand_transfer'].copy()
        brand_transfer_df.index.name = "品牌"
        brand_transfer_df.to_csv(f"{IMAGE_SAVE_PATH}\\品牌过户分析.csv", encoding='utf-8-sig')
        print("  ✓ 品牌过户分析数据已保存")

    if not profit_feature_importance.empty:
        profit_feature_importance.to_csv(f"{IMAGE_SAVE_PATH}\\利润预测特征重要性.csv", index=False, encoding='utf-8-sig')
        print("  ✓ 利润预测特征重要性数据已保存")

    print(f"  ✓ 利润过户分析结果已保存到: {IMAGE_SAVE_PATH}")
except Exception as e:
    print(f"  ✗ 保存数据失败: {e}")
# ==================================================================
