...
# 修改后的XLSX文件读取路径（按示例格式调整）
FILE_PATHS = {
    'range': r"D:\HuaweiMoveData\Users\江磊\Desktop\电动汽车销售\电动汽车销售指标表\续航里程分析.xlsx",
    'battery': r"D:\HuaweiMoveData\Users\江磊\Desktop\电动汽车销售\电动汽车销售指标表\电池容量分析.xlsx",
    'price': r"D:\HuaweiMoveData\Users\江磊\Desktop\电动汽车销售\电动汽车销售指标表\价格区间分析.xlsx",
    'drive': r"D:\HuaweiMoveData\Users\江磊\Desktop\电动汽车销售\电动汽车销售指标表\驱动形式分析.xlsx",
    'smart': r"D:\HuaweiMoveData\Users\江磊\Desktop\电动汽车销售\电动汽车销售指标表\智能化水平分析.xlsx",
    'charging': r"D:\HuaweiMoveData\Users\江磊\Desktop\电动汽车销售\电动汽车销售指标表\充电效率分析.xlsx"
}
...
def feature_importance_analysis(df):
    ...
    print("\n  特征重要性排名:")
    for rank, (i, row) in enumerate(importance_df.head(5).iterrows(), 1):
    ...
    top_features = filtered_importance.head(5).copy()  # 加 .copy() 避免 SettingWithCopyWarning
    # 映射为中文列名
...
def generate_report(df, corr_matrix, cluster_model, centers, importance, price_insights, trend_insights, profiles):
    ...
    if not importance.empty:
        ...
        for rank, (i, row) in enumerate(importance.head(5).iterrows(), 1):
            report.append(f"    {rank}. {CSV_COLUMN_MAPPING.get(row['feature'], row['feature'])}: {row['importance']:.3f}")
        report.append("")
