...
from adjustText import adjust_text  # 新增：导入调整文本的库
import warnings
...
# ==============================================
# 1. 优先使用系统中最稳定的中文字体
plt.rcParams['font.sans-serif'] = ['Microsoft YaHei', 'SimHei', 'WenQuanYi Zen Hei', 'Heiti TC', 'Arial Unicode MS', 'Deja Sans']
plt.rcParams['axes.unicode_minus'] = False
plt.rcParams['font.family'] = 'sans-serif'
plt.rcParams['axes.formatter.use_mathtext'] = False

# 2. 强制所有文本使用无衬线字体，避免特殊字符渲染失败
    ...
    if (analysis_results['sales_trend'] is not None and
        ...
        try:
            ...
            region_analysis = pd.DataFrame({
                '销售总量': region_sales,
                '总利润': brand_profit['总利润'],
                '平均利润率': brand_profit['平均利润率'],
                '总过户数量': brand_transfer['总过户数量']
            }).fillna(0)
...
def principal_component_analysis():
    ...
    if correlations.get('region') is not None and len(correlations['region']['data']) > 1:
        try:
            ...
            # 执行PCA
            pca = PCA()
            pca_result = pca.fit_transform(StandardScaler().fit_transform(region_data))
...
def plot_comprehensive_dashboard():
    ...
    if analysis_results['customer'] is not None:
        ...
        for i, bar in enumerate(bars):
            ...
            ax2.text(width + width*0.01, bar.get_y() + bar.get_height()/2,
                     f'¥{width:.1f}万', ha='left', va='center', fontsize=10, fontfamily='Microsoft YaHei')
    ...
    if analysis_results['product'] is not None:
        ...
        for i, bar in enumerate(bars):
            ...
            ax3.text(width + width*0.01, bar.get_y() + bar.get_height()/2,
                     f'{width:.3f}', ha='left', va='center', fontsize=10, fontfamily='Microsoft YaHei')
    ...
    if correlations.get('region') is not None:
        ...
        # --- 核心修复：使用adjust_text自动调整重叠的标签 ---
        texts = []
        for i, (region, row) in enumerate(region_data.iterrows()):
            # 获取数据点的坐标
            x, y = row['销售总量'], row['总利润']
            # 添加标签到列表
            texts.append(ax5.text(x, y, region, fontsize=9, fontfamily='Microsoft YaHei', ha='center', va='center'))
    ...
    ax8.text(0.02, 0.98, insight_text, transform=ax8.transAxes, fontsize=13,
             verticalalignment='top', fontfamily='Microsoft YaHei',  # 明确指定为微软雅黑
...
# 保存地区聚类结果
if clustering_results.get('region') is not None:
    clustering_results['region']['data'].to_csv(f"{IMAGE_SAVE_PATH}\\地区综合聚类结果.csv", index=True, encoding='utf-8-sig')
    print(f"  ✓ 地区综合聚类结果已保存")

# 保存跨模块关联数据
if correlations.get('region') is not None:
    correlations['region']['data'].to_csv(f"{IMAGE_SAVE_PATH}\\地区跨模块关联数据.csv", index=True, encoding='utf-8-sig')
    correlations['region']['correlation_matrix'].to_csv(f"{IMAGE_SAVE_PATH}\\地区跨模块相关性矩阵.csv", index=True, encoding='utf-8-sig')
    print(f"  ✓ 跨模块关联数据已保存")
