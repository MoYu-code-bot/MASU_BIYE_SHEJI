...
# 设置图片保存路径
IMAGE_SAVE_PATH = r"D:\Masu_BiYe_sheji\Python_SuanFa\electric_car_sale\销售趋势分析算法图片存储"
...
def load_sales_trend_data():
    ...
    """
    # 加载销售数据
    monthly_df = pd.read_excel(r"D:\HuaweiMoveData\Users\江磊\Desktop\电动汽车销售\AI文件\月度销售趋势.xlsx")
    quarterly_df = pd.read_excel(r"D:\HuaweiMoveData\Users\江磊\Desktop\电动汽车销售\AI文件\季度销售趋势.xlsx")
    yearly_df = pd.read_excel(r"D:\HuaweiMoveData\Users\江磊\Desktop\电动汽车销售\AI文件\年度销售趋势.xlsx")
    regional_df = pd.read_excel(r"D:\HuaweiMoveData\Users\江磊\Desktop\电动汽车销售\AI文件\地区销售趋势.xlsx")
    ...
    for df in [monthly_df, quarterly_df, yearly_df, regional_df]:
        if '统计月份' in df.columns:
            df['统计月份'] = pd.to_datetime(df['统计月份'])
        elif '统计季度' in df.columns:
            df['统计季度'] = pd.to_datetime(df['统计季度'])
        elif '统计年度' in df.columns:
            df['统计年度'] = pd.to_datetime(df['统计年度'], format='%Y')
...
def plot_sales_trend_analysis():
    ...
    if monthly_results and len(monthly_results) > 0:
        ...
        bars1 = axes[0].bar(x - width, mse_scores, width, label='均方误差(MSE)', color='red', alpha=0.7)
        bars2 = axes[0].bar(x, mae_scores, width, label='平均绝对误差(MAE)', color='blue', alpha=0.7)
        bars3 = axes[0].bar(x + width, r2_scores, width, label='决定系数(R²)', color='green', alpha=0.7)

        axes[0].set_xlabel('模型')
        axes[0].set_ylabel('评分值')
...
with open(f"{IMAGE_SAVE_PATH}\\销售趋势分析报告.txt", 'w', encoding='utf-8') as f:
    f.write(report_content)
print(f"\n✓ 分析报告已保存: {IMAGE_SAVE_PATH}\\销售趋势分析报告.txt")
