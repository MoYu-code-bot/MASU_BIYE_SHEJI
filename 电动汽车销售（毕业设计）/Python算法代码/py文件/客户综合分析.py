...
def load_customer_data():
    ...
    """
    # 加载三个客户数据文件（修改后的路径）
    age_df = pd.read_excel(r"D:\HuaweiMoveData\Users\江磊\Desktop\电动汽车销售\电动汽车销售指标表\客户年龄分析.xlsx")
    gender_df = pd.read_excel(r"D:\HuaweiMoveData\Users\江磊\Desktop\电动汽车销售\电动汽车销售指标表\客户性别分析.xlsx")
    occupation_df = pd.read_excel(r"D:\HuaweiMoveData\Users\江磊\Desktop\电动汽车销售\电动汽车销售指标表\客户职业分析.xlsx")
...
def analyze_feature_importance(model, feature_cols):
    ...
    if hasattr(model, 'feature_importances_'):
        ...
        plt.xlabel('重要性得分', fontfamily=CHINESE_FONT)
        plt.title('客户行为预测特征重要性', fontfamily=CHINESE_FONT)
        plt.gca().invert_yaxis()
        plt.gca().set_yticklabels(importance_df['feature'], fontfamily=CHINESE_FONT)

        # 添加数值标签
        for bar in bars:
            width = bar.get_width()
            plt.text(width + 0.01, bar.get_y() + bar.get_height() / 2,
                     f'{width:.3f}', ha='left', va='center', fontsize=9, fontfamily=CHINESE_FONT)
...
def generate_customer_report():
    ...
    if feature_importance is not None:
        ...
        for rank, (i, row) in enumerate(feature_importance.head(3).iterrows(), 1):
            report.append(f"    {rank}. {row['feature']}: {row['importance']:.3f}")
        report.append("")
...
if feature_importance is not None:
    ...
    feature_importance_cn.to_csv(f"{DOCUMENT_SAVE_PATH}\\特征重要性分析.csv", index=False, encoding='utf-8-sig')
...
print(f"  ✓ 聚类结果、CLV数据和特征重要性数据已保存为CSV格式到: {DOCUMENT_SAVE_PATH}")
